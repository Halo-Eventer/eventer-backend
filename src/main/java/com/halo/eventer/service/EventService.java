...
import com.halo.eventer.dto.event.GetAllEventDto;
...
public class EventService {
    ...
    public String createEvent(EventCreateDto eventCreateDto, Long festivalId) throws Exception {
        ...
        List<Image> images = eventCreateDto.getImages().stream().map(o-> new Image(o)).collect(Collectors.toList());
    ...
    public EventResDto getEvent(Long eventId) throws Exception {
        ...
        EventResDto response = new EventResDto(event);
        response.setImages(event.getImages());
        return response;
    }

    public List<GetAllEventDto> getEvents(Long festivalId) throws Exception {
        List<GetAllEventDto> response = eventRepository
                .findAllByFestival(festivalRepository.findById(festivalId).orElseThrow(() -> new Exception("존재하지 않습니다.")))
                .stream().map(o -> new GetAllEventDto(o)).collect(Collectors.toList());
